...
exports.handler = function(event, context) {
    ...
    console.log("DFUserAuthCreateUser - just about to computeHash()...");
    
    computeHash(clearPassword, function(err, salt, hash) {
        if (err) {
            context.fail('422: Unprocessable Entity - Error in hash: ' + err);
        } else {
            console.log("DFUserAuthCreateUser - just about to storeUser()...");
            
            storeUser(email, hash, salt, function(err, token) {
                if (err) {
                    if (err.code == 'ConditionalCheckFailedException') {
                        console.log("DFUserAuthCreateUser - userId already found?");
                        ...
                        context.fail('403: Forbidden - Error in storeUser: ' + err);
                    }
                } else {
                    sendVerificationEmail(email, token, function(err, data) {
                        if (err) {
                            context.fail('422: Unprocessable Entity - Error in sendVerificationEmail: ' + err);
                        } else {
                            console.log("DFUserAuthCreateUser - sendVerificationEmail() succeeded!?");
