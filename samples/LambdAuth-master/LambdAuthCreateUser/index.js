...
exports.handler = function(event, context) {
    ...
    console.log("LambdaAuthCreateUser - just about to computeHash()...");
    
	computeHash(clearPassword, function(err, salt, hash) {
		if (err) {
			context.fail('Error in hash: ' + err);
		} else {
            console.log("LambdaAuthCreateUser - just about to storeUser()...");
            
			storeUser(email, hash, salt, function(err, token) {
				if (err) {
					if (err.code == 'ConditionalCheckFailedException') {
                        console.log("LambdaAuthCreateUser - userId already found?");
					...
					sendVerificationEmail(email, token, function(err, data) {
						if (err) {
                            ...
                            console.log("LambdaAuthCreateUser - sendVerificationEmail() succeeded!?");
